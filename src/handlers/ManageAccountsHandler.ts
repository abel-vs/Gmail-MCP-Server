...
/**
 * Unified handler for managing Gmail accounts.
 *
...
export class ManageAccountsHandler {
  ...
  private async listAccounts(accountId: string | undefined, context: ServerContext): Promise<CallToolResult> {
    ...
    if (accounts.size === 0) {
      const response: AccountStatusResponse = {
        accounts: [],
        total_accounts: 0,
        message: 'No authenticated accounts found. Use manage-accounts with action "add" and provide a nickname to connect a Gmail account.'
      };
  ...
  private async addAccount(accountId: string | undefined, context: ServerContext): Promise<CallToolResult> {
    ...
    process.env.GMAIL_ACCOUNT_MODE = normalizedId;
    ...
    try {
      ...
      const response: AddAccountResponse = {
        ...
        instructions: `Visit the auth_url in your browser to connect your Gmail account. This will be saved with the nickname "${normalizedId}".`,
